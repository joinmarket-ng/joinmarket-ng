...
Constructs a :class:`~jm_tumbler.plan.Plan` from a list of destination
...
* **Stage 1 - origin cleavage.** For every non-empty mixdepth, schedule one
  :class:`~jm_tumbler.plan.TakerCoinjoinPhase` sweep to an internal address
...
* **Stage 2 - destination mixing.** For each destination, schedule a
  ...
  :class:`~jm_tumbler.plan.MakerSessionPhase` sessions and
  :class:`~jm_tumbler.plan.BondlessTakerBurstPhase` sub-bursts, and a final
...
class TumbleParameters:
    ...
    These are copied into :class:`~jm_tumbler.plan.PlanParameters` on the plan
...
@dataclass
class PlanBuilder:
    """Turns :class:`TumbleParameters` into a :class:`~jm_tumbler.plan.Plan`."""
