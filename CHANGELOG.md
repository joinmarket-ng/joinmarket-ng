...
### Changed
- **Taker fee randomization default too aggressive**: Changed `tx_fee_factor` default from `3.0` to `0.2` to match JoinMarket reference behavior (`tx_fees_factor = 0.2`). Fee randomization now defaults to a +20% band instead of up to +300%, avoiding unexpectedly high miner fee rates.
