...
- **Flatpak packaging**: Self-contained Linux desktop application bundling jmwalletd, JAM web frontend, Tor daemon, neutrino-api light client, and orderbook watcher. Install via `.flatpak` bundle attached to releases. Entrypoint script manages process lifecycle (Tor, neutrino, jmwalletd, obwatcher) with readiness checks. Configurable via `~/.joinmarket-ng/config.toml`: use bundled or system Tor, bundled or external neutrino, or full Bitcoin Core. GitHub Actions workflow builds and attaches `.flatpak` bundles to releases.
  - **GTK3 control panel**: GUI using PyGObject/GTK3 (from the freedesktop runtime) with native Wayland and X11 support. Features: open-browser button, quit button, log viewer dropdown (jmwalletd/Tor/neutrino/obwatcher) with auto-refresh tailing, network badge, and dark theme. Replaces the initial tkinter prototype which was X11-only and crashed on Wayland-only sessions (Flatpak's `--socket=fallback-x11` does not expose `$DISPLAY` when Wayland is available).
  - **Network selection**: `--network mainnet|signet|regtest` flag configures both joinmarket-ng services and neutrino.
  - **CLI passthrough**: `flatpak run org.joinmarketng.JamNG cli <command> [args...]` runs any console script (jm-wallet, jm-taker, jm-maker, etc.) inside the sandbox with the running instance's environment.
  - **Dynamic port allocation**: All service ports (Tor SOCKS, Tor control, neutrino, jmwalletd) are dynamically allocated. jmwalletd prefers port 28183 but falls back to a random free port if it is already in use (e.g. by a Docker Compose test stack), so the Flatpak never conflicts with other local services. The GUI and status output always reflect the actual port in use.
...
- **jmwalletd SPA catch-all returning 405 for non-GET requests**: The fallback route for serving the JAM single-page application was registered as `@app.get("/{full_path:path}")`, which caused FastAPI to return `405 Method Not Allowed` for POST/PUT/DELETE requests to undefined API paths (instead of the correct `404 Not Found`). Replaced with a `starlette.routing.Route` restricted to `GET` and `HEAD` methods, appended to `app.router.routes` so it does not interfere with API endpoint routing.
