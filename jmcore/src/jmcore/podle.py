...
def verify_podle_binding(p: bytes, scriptpubkey: bytes | str) -> tuple[bool, str]:
    ...
    Supported script types: P2PKH, P2SH-P2WPKH, P2WPKH (all using the 33-byte
    compressed ``P``) and P2TR (BIP86 key-path, comparing the x-only key).
    ...
    if len(spk) == 34 and spk[0] == 0x51 and spk[1] == 0x20:
        if spk[2:] == p[1:]:
            return True, ""
        return False, "P does not match P2TR scriptpubkey"
