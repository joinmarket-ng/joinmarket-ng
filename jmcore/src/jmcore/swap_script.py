...
This module lives in ``jmcore`` because the swap HTLC script is a shared
Bitcoin protocol primitive consumed by both the taker (which acquires swap
inputs) and the wallet (which owns the swap claim keys). It depends only on
``jmcore.bitcoin`` and the standard library, so neither component needs to
depend on the other.
"""
