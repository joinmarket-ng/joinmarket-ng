...
def _taker(tmp_path: Path, **overrides: Any) -> Taker:
    wallet = AsyncMock()
...
def test_market_fault_exclusion_requires_opt_in(tmp_path: Path, enabled: bool | None) -> None:
    # Trading and sanctions are separate opt-ins: enforcement can be enabled
    # without buying credentials, but missing settings preserve maker selection.
    taker = _taker(tmp_path, **({} if enabled is None else {"market_fault_exclusion": enabled}))
    raw, bond = _proof()
    ...
    expected = ["untainted"] if enabled else ["matching-one", "matching-two", "untainted"]
    assert [offer.counterparty for offer in filtered] == expected
    selected, _fee = taker.orderbook_manager.select_makers(cj_amount=1, n=3)
    assert set(selected) == set(expected)
    assert (tmp_path / "market_faults.json").exists() is bool(enabled)


def test_disabled_exclusion_does_not_load_or_modify_existing_fault_cache(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    raw, bond = _proof()
    offer = _offer(bond, "matching")
    cache = MarketFaultCache(tmp_path)
    assert cache.ingest(raw)
    assert cache.excluded_nicks([offer], network="regtest", height=2016) == {"matching"}
    path = tmp_path / "market_faults.json"
    original = path.read_bytes()
    read = Mock(side_effect=AssertionError("disabled taker must not read fault cache"))
    monkeypatch.setattr("jmcore.market_faults.read_private_file", read)
    taker = _taker(tmp_path)
    assert taker._drain_and_apply_market_faults([offer], 2016) == [offer]
