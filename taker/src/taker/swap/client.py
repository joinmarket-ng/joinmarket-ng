...
class SwapClient:
    ...
    async def cancel_pending_payment(self) -> None:
        """Cancel the in-flight main hold-invoice payment, if any.

        Used when the CoinJoin fails AFTER the swap provider has broadcast the
        on-chain lockup. Cancelling the asyncio task stops awaiting the LND
        REST call locally; the in-flight HTLC at the LN layer will be failed
        by LND once its CLTV expires (the preimage is never revealed because
        we never broadcast a CoinJoin spending the lockup).

        Note: the prepay (miner-fee) invoice is already settled at this point
        and cannot be reclaimed. This loss is the trade-off for cleanly
        aborting the round; see docs/technical/swap_input_privacy.md.
        """
        task = self._main_payment_task
        if task is None or task.done():
            return
        logger.warning(
            "Cancelling in-flight main hold-invoice payment. "
            "Prepay invoice is forfeit; HTLC will fail at CLTV expiry."
        )
        task.cancel()
        try:
            await task
        except (asyncio.CancelledError, Exception) as exc:  # noqa: BLE001
            logger.debug(f"Main payment task ended after cancel: {exc!r}")
        finally:
            self._main_payment_task = None
    ...
    async def _pay_invoice(self, payment_request: str, timeout_seconds: int = 60) -> None:
        ...
        try:
            ...
            # MPP can improve Lightning privacy (Kappos et al. 2021) by
            # spreading routing observation across paths. It only helps when
            # the sender has multiple usable channels; with a single direct
            # channel to the recipient, splitting just creates extra HTLCs on
            # the same channel and complicates failure handling. We request
            # up to 4 parts (LND will use 1 if it cannot improve) and mark
            # the payment cancelable so the local timeout aborts in-flight
            # attempts cleanly.
    ...
    async def _wait_for_lockup(
        ...
        Detection strategy:
        Poll ``backend.scan_external_address(lockup_address)`` which scans the
        global UTXO set (via ``scantxoutset`` on Bitcoin Core backends) and
        therefore works for addresses that are *not* tracked by the local
        wallet — the swap provider's lockup address always falls into that
        category.

        ``scantxoutset`` only returns confirmed UTXOs, but the lockup
        typically confirms within a few minutes in regtest (auto-mining) and
        within ~10 minutes on mainnet.
        ...
        while time.monotonic() - start_time < timeout:
            try:
                utxos = await self.backend.scan_external_address(lockup_address)
