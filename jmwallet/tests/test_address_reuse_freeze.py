...
def test_freezes_utxo_on_already_used_address(tmp_path: Path) -> None:
    ...
    frozen = ws._auto_freeze_reused_address_utxos({REUSED_ADDRESS}, set())
...
def test_does_not_freeze_pre_existing_utxo_at_reused_address(tmp_path: Path) -> None:
    """The original deposit at a (now-reused) address must stay spendable.

    Only a *newly arrived* UTXO is frozen; a pre-existing one (in
    ``prior_known_outpoints``) is left alone even though its address is in the
    used set. This mirrors legacy's "freeze the reuse, keep the original".
    """
    ws = _make_wallet(tmp_path, max_sats_freeze_reuse=-1)
    ...
    # The address is already used; the original is pre-existing, the reuse is new.
    frozen = ws._auto_freeze_reused_address_utxos(
        {REUSED_ADDRESS}, prior_known_outpoints={original.outpoint}
    )

    assert frozen == 1
    assert original.frozen is False, "the original deposit must remain spendable"
    assert reuse.frozen is True, "the newly arrived reuse UTXO must be frozen"
...
def test_first_sight_of_multiple_utxos_freezes_nothing(tmp_path: Path) -> None:
    ...
    assert ws._auto_freeze_reused_address_utxos(set(), set()) == 0
...
def test_does_not_override_user_unfrozen_reuse_utxo(tmp_path: Path) -> None:
    ...
    assert ws._auto_freeze_reused_address_utxos({REUSED_ADDRESS}, set()) == 1
    assert utxo.frozen is True
    ...
    assert ws._auto_freeze_reused_address_utxos({REUSED_ADDRESS}, set()) == 0
    assert utxo.frozen is False
