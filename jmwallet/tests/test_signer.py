...
class TestSignInputTaprootBond:
    def test_taproot_bond_script_path_witness(self, wallet_service):
        from jmcore.btc_script import derive_taproot_bond_address

        from jmwallet.wallet.signing import verify_p2tr_script_path_signature

        address = wallet_service.get_taproot_fidelity_bond_address(BOND_LOCKTIME)
        bond_key = wallet_service.get_taproot_fidelity_bond_key(BOND_LOCKTIME)
        xonly = bond_key.get_public_key_bytes(compressed=True)[1:]
        bond = derive_taproot_bond_address(xonly, BOND_LOCKTIME, wallet_service.network)
        ...
        prevout_scripts = [bond.scriptpubkey]

        signed = wallet_service.sign_input(
            tx, 0, utxo, prevout_values=prevout_values, prevout_scripts=prevout_scripts
        )

        # BIP342 witness stack: [signature, tapleaf_script, control_block].
        assert len(signed.signature) == 64
        assert signed.witness == [signed.signature, bond.tapleaf_script, bond.control_block]
        assert signed.pubkey == xonly
        assert verify_p2tr_script_path_signature(
            tx, 0, prevout_values, prevout_scripts, signed.signature, xonly, bond.tapleaf_script
        )

    def test_taproot_bond_requires_prevouts(self, wallet_service):
        address = wallet_service.get_taproot_fidelity_bond_address(BOND_LOCKTIME)
        bond_key = wallet_service.get_taproot_fidelity_bond_key(BOND_LOCKTIME)
        from jmcore.btc_script import derive_taproot_bond_address
        ...
        tx = _single_input_tx()
        with pytest.raises(TransactionSigningError, match="full prevout set"):
            wallet_service.sign_input(tx, 0, utxo)
