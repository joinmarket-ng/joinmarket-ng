...
class TestDescriptorRangeUpgrade:
    ...
    @pytest.mark.asyncio
    async def test_setup_auto_expand_no_expansion_when_within_range(self) -> None:
        """Empty wallet (no used addresses): probe finds nothing, no expansion."""
        from jmwallet.wallet.service import WalletService

        backend = DescriptorWalletBackend(wallet_name="test_autoexp_empty")
        backend._wallet_loaded = True
        ...
        async def mock_rpc(
            ...
            if method == "listwallets":
                return ["test_autoexp_empty"]
            if method == "listdescriptors":
                return {"descriptors": []}
            if method == "getdescriptorinfo":
                desc = params[0] if params else ""
                return {"descriptor": f"{desc}#mockcs"}
            if method == "importdescriptors":
                return [{"success": True} for _ in (params[0] if params else [])]
            if method == "getblockchaininfo":
                return {"blocks": 100}
        ...
        await wallet.setup_descriptor_wallet(
            rescan=True, smart_scan=False, background_full_rescan=False, check_existing=False
        )

        # Probe ran but did not expand
        assert "listreceivedbyaddress" in rpc_calls
        assert wallet.scan_range == 1000

    @pytest.mark.asyncio
    async def test_setup_auto_expand_grows_on_deep_history(self) -> None:
        """Used address near the scan_range boundary triggers a single expansion."""
        from jmwallet.wallet.service import WalletService

        backend = DescriptorWalletBackend(wallet_name="test_autoexp_grow")
        backend._wallet_loaded = True
        ...
        # Pre-derive an address right at the edge (index 990 with gap_limit 20
        # means highest_used >= 980 triggers expansion).
        deep_addr = wallet.get_address(0, 0, 990)
        wallet.address_cache.clear()  # force re-derivation via _populate_address_cache

        upgrade_ranges: list[int] = []
        ...
        async def mock_rpc(
            ...
            if method == "getdescriptorinfo":
                ...
                return {"descriptor": f"{desc}#mockcs"}
            if method == "importdescriptors":
                # Record the requested range end for ranged descriptors
                if params:
                    for desc in params[0]:
                        if "range" in desc:
                            upgrade_ranges.append(desc["range"][1])
                return [{"success": True} for _ in (params[0] if params else [])]
            if method == "getblockchaininfo":
                return {"blocks": 100}
            if method == "listreceivedbyaddress":
                # First probe reports the deep address; after expansion to
                # 2000, the address sits well within range so no further
                # expansion.
                return [{"address": deep_addr, "amount": 0.01, "txids": ["a" * 64]}]
            if method == "rescanblockchain":
                return {"start_height": 0, "stop_height": 100}
            return {}
        ...
        # Initial import at 999, expansion at 1999
        assert 999 in upgrade_ranges
        assert 1999 in upgrade_ranges
        assert wallet.scan_range == 2000

    @pytest.mark.asyncio
    async def test_setup_auto_expand_respects_max_bound(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Adversarial used-address at each expansion boundary stops at bound."""
        from jmwallet.wallet import sync as sync_module
        from jmwallet.wallet.service import WalletService

        # Shrink the cap so the test stays fast; the logic under test is the
        # same regardless of the absolute value.
        small_max = 4000
        monkeypatch.setattr(sync_module, "MAX_AUTO_SCAN_RANGE", small_max)

        backend = DescriptorWalletBackend(wallet_name="test_autoexp_bound")
        backend._wallet_loaded = True
        ...
        # Pre-derive addresses just below each successive doubling boundary so
        # the probe finds a fresh "deep" address after every expansion and is
        # forced to keep doubling until the bound kicks in.
        boundary_addrs: dict[int, str] = {}
        r = 1000
        while r <= small_max:
            boundary_addrs[r] = wallet.get_address(0, 0, r - 5)
            r *= 2
        wallet.address_cache.clear()
        ...
        async def mock_rpc(
            ...
            if method == "listreceivedbyaddress":
                return [
                    {"address": addr, "amount": 0.01, "txids": ["a" * 64]}
                    for addr in boundary_addrs.values()
                ]
            if method == "rescanblockchain":
                return {"start_height": 0, "stop_height": 100}
            return {}
        ...
        # Final range is capped at the (patched) bound
        assert wallet.scan_range == small_max

    @pytest.mark.asyncio
    async def test_setup_auto_expand_skipped_without_rescan(self) -> None:
        """``rescan=False`` skips the probe entirely (no history to read)."""
        from jmwallet.wallet.service import WalletService

        backend = DescriptorWalletBackend(wallet_name="test_autoexp_norescan")
        backend._wallet_loaded = True
        ...
        await wallet.setup_descriptor_wallet(
            rescan=False, smart_scan=False, background_full_rescan=False, check_existing=False
        )

        # Probe must not have run
        assert "listreceivedbyaddress" not in rpc_methods
        assert wallet.scan_range == 1000
