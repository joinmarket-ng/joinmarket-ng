...
class WalletService(
    ...
    def get_fidelity_bond_path(self, index: int) -> str:
        """P2WSH bond path, always under BIP84 so both pits share the same bonds."""
        return f"{self.fidelity_bond_root_path}/0'/{FIDELITY_BOND_BRANCH}/{index}"
    ...
    async def create_market_authorization(
        ...
        expected_path = self.get_fidelity_bond_path(timenumber)
    ...
    async def create_market_bond_credential(
        ...
        # its derived address and chain UTXO; sign the lease with that bond key.
        matches = [
            utxo
            for utxo in self.utxo_cache.get(0, [])
        ...
        bond_key = self.get_fidelity_bond_key(0, fresh_authority.bond.locktime)
