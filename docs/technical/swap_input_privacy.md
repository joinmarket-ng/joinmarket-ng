...
5. Distributes the leftover sats so every "fee" output (real maker fees +
   the taker's fake fee) equals `max(maker_fee)`. Any remainder is burned to
   miners. Partial top-ups are intentionally suppressed because non-orderbook
   fee values are themselves a fingerprint.
...
- **Tor-by-default for the LND REST connection.** When the configured
  `lnd_rest_url` is a `.onion` and a SOCKS proxy is configured (the same one
  used for Nostr / directory-server traffic), the LND REST client routes
  through it. Operators with a remote LND should always run it behind a
  hidden service.
- **MPP (multi-path payments).** The taker pays the main hold invoice via
  LND `SendPaymentV2` with `max_parts=16` so the routing observation is
